...
import { GET_TOP_SCORES, GET_RANK_FROM_SCORE } from "../../shortcuts/requests"
...
export default class Leaderboard extends Phaser.Scene {
  ...
  async create() {
    ...
    this.me.bar = this.addScoreBar(this.game.GH - 70)
      .setTexture("general-3")
      .setFrame("lb-me-bar")
  ...
  createPodiumSet() {
    ...
    const aura = this.add.image(
      this.game.GW / 2,
      upper_strip.y + upper_strip.displayHeight + 170,
      "podium-aura"
    )
    this.tweens.add({
      targets: aura,
    ...
    this.add.image(
      this.game.GW / 2,
      upper_strip.y + upper_strip.displayHeight + 230,
      "podium-glow"
    )

    this.podium = this.add
      .image(
        this.game.GW / 2,
        upper_strip.y + upper_strip.displayHeight + 150,
        "podium"
      )
      .setOrigin(0.5, 0)
  }
  ...
  updateTexts(_sorted_data) {
    ...
    this.texts.forEach((account_text, i) => {
      ...
      for (let i = 1; i <= this.leaderboard_shift_value; ++i) {
        if (acc.rank == i) {
      ...
      if (acc.nickname === my_nickname) {
        if (acc.rank <= this.leaderboard_shift_value) {
...
*/
  ...
  createMedals() {
    ...
    for (let i = 0; i < this.leaderboard_shift_value; ++i) {
      this.medalManager.medals[i] = this.add.image(
        medal_x,
        this.texts[i].rank.y + 7,
        "general-3",
        images[i]
      )
    }
    ...
    try {
      return await GET_RANK_FROM_SCORE({
        level: Utils.convertLevelNumberToLevelName(
          levelsConfiguration[this.level - 1]
        ),
        score: window.progress.levels_scores[this.level - 1],
      })
    } catch {
  ...
  createLevelInfo() {
    ...
    this.add
      .text(this.game.GW / 2, y, difficulty.toUpperCase(), {
        font: `65px ${main_font}`,
      })
      .setOrigin(0.5)
    ...
    this.add
      .image(this.game.GW / 2 + bar.displayWidth / 2, y, "levels-icons", icon)
      .setDepth(0.1)
    this.add.image(
      this.game.GW / 2 + bar.displayWidth / 2,
      y,
      "general-1",
      "glow"
    )
  ...
  calculateVariablesHeightDependend() {
    const bar_height =
      this.game.textures.list["general-1"].frames[this.lb_texture].cutHeight

    const constant_difference = 55
    const bottom_space = bar_height
    const upper_space =
      this.podium.y + this.podium.displayHeight + bar_height / 2
    const empty_space =
      this.game.GH - bottom_space - upper_space - constant_difference * 2
    ...
    this.score_bg_y =
      this.first_bar_y + (bars_amount * bar_height) / 2 - bar_height / 2
    this.score_bg_height = bar_height * bars_amount + 30

    this.gold_line_y =
      this.me.bar.y -
      this.me.bar.displayHeight / 2 -
      (this.me.bar.y -
        this.me.bar.displayHeight / 2 -
        (this.first_bar_y + this.score_bg_height - bar_height / 2)) /
        2 -
      10
